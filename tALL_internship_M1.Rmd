...
paths <- list.files(
    path = 'GDCdata/TARGET-ALL-P2/harmonized/Transcriptome_Profiling/Gene_Expression_Quantification/',
    full.name = TRUE,
    recursive = TRUE,
    pattern = '*.tsv'
  )
...
write.table(m, "RNASeq_P2.csv")
```

```{r}
############################################################################
###########   Annie's attempt to extract data from expression data  P3    #### 
############################################################################

#####Pull the name of patient(cases) in this study####### 
P3_mRNA_cases <- query_mRNA_P3[[1]][[1]]$cases

#####"get list of interesting files
paths_P3 <- list.files(
    path = 'C:/Users/nguye/OneDrive/Bureau/internshipDLAD_M1/Rscripts/GDCdata/TARGET-ALL-P3/harmonized/Transcriptome_Profiling/Gene_Expression_Quantification/',
    full.name = TRUE,
    recursive = TRUE,
    pattern = '*.tsv'
  )
...
#the short name of each file: sample ID
sample.names_P3 <- substr(basename(paths_P3), 1, 36)

sample.names_P3 <- P3_mRNA_cases

#get transcript names from first file: paths[1]
#skip the first 6 rows (uninteresting)
#transcript.names_P3 <- read.delim(paths_P3[1], skip = 6, header = FALSE)[1][[1]] #get only the 1st column [1] then extract to vector [[1]]
gene.names_P3 <- read.delim(paths_P3[1], skip = 6, header = FALSE)[2][[1]] #get only the 1st column [1] then extract to vector [[1]]

#their number  
nb.transcripts_P3 <- length(transcript.names_P3)
...
for (i in 1:length(paths_P3)) {
  ...
  m_P3[,i]  <- read.delim(paths_P3[[i]], skip = 6, header = FALSE)[4][[1]]
}


write.table(m_P3, "RNASeq_P3.csv")
```
...
############################################
## Step 1 - create countdata and colData ###
############################################

#1.1 count data 
P2_mRNA_count_data <- read.csv("C:/Users/nguye/OneDrive/Bureau/internshipDLAD_M1/R_scripts/RNASeq_P2.csv", header = FALSE, sep = "\t")
as.data.frame(P2_mRNA_count_data, sep = "\t")
head(P2_mRNA_count_data)

#1.2 colData = sample's info 
P2_mRNa_colData <- read_xlsx("C:/Users/nguye/OneDrive/Bureau/internshipDLAD_M1/R_scripts/GDCdata/TARGET-ALL-P2/harmonized/Clinical/TARGET_ALL_ClinicalData_Phase_II_Validation_20211118_modified.xlsx")
P2_mRNa_colData <- data.frame(sample = P2_mRNa_colData$`TARGET USI`, type = P2_mRNa_colData$`Cell of Origin`)


#making sure the rowname in colData match the column name in countdata
